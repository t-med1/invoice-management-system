...
<!-- Le contenu du formulaire à remplir -->
<?php $this->section('content') ?>
    <?php if (isset($result)): ?>
        <table class="table table-light table-hover table-striped rounded" style="overflow: hidden" id='factures'>
            ...
            <tbody>
                <?php
                $count = 0; // Initialise la variable $count
                $displayedFactures = []; // Tableau pour stocker les identifiants de facture déjà affichés
                ?>
                <?php foreach ($result as $fact): ?>
                    <?php
                    $facture_id = $fact['id_facture'];
                    if (in_array($facture_id, $displayedFactures)) {
                        continue; // Si la facture est déjà affichée, passer à l'itération suivante
                    }
                    $displayedFactures[] = $facture_id; // Ajouter l'identifiant de facture au tableau des factures déjà affichées
                    ?>
                    <tr>
                        ...
                        <td><?= isset($fact['date_dernier_paiment']) ? $fact['date_dernier_paiment'] : '00-00-00' ?></td>
                        <td><?= isset($fact['montant_paye']) ? $fact['montant_paye'] : '00.00' ?></td>
                        <td><?= $fact['status'] ?></td>
                        <td><?= isset($fact['status_litige']) ? $fact['status_litige'] : '' ?></td>
                        <td><?= isset($fact['status_paiment']) ? $fact['status_paiment'] : '' ?></td>
                        <td>
            ...
            btn.addEventListener('click', e => {
                ...
                html2canvas(document.getElementById('devis' + e.target.id))
                    .then(canvas => {
                        ...
                        pdf.addImage(base64image, 'PNG', 4, -50, 1117, 1500);
